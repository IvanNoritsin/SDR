...
// The cf-ad9361-dds-core-lpc is the DAC/TX output driver that controls the TX DMA and the TX HDL core, including the DDS.
    ...
    std::ofstream outfile_i("single_adalm_rx_I_arr.txt", std::ios::out);
    std::ofstream outfile_q("single_adalm_rx_Q_arr.txt", std::ios::out);
    ...
    if (outfile_i.is_open()){
        while (getline(outfile_i, tx_i)){
            tx_i[count] = static_cast<int16_t>(stoi(line));
            ++count
        }
    }
    ...
    if (outfile_q.is_open()){
        while (getline(outfile_q, tx_q)){
            tx_q[count] = static_cast<int16_t>(stoi(line));
            ++count
        }
    }
