...
int main(){
    ...
    std::ifstream infile_i("single_adalm_rx_I_arr.txt", std::ios::in);
    std::ifstream infile_q("single_adalm_rx_Q_arr.txt", std::ios::in);

    int16_t tx_i[250];
    int16_t tx_q[250];

    if (infile_i.is_open() && infile_q.is_open()) {
        for (int i = 0; i < 250; ++i) {
            if (!(infile_i >> tx_i[i])) {
                std::cerr << "Ошибка при чтении данных из файла single_adalm_rx_I_arr.txt на индексе " << i << std::endl;
                return 1;
            }
            if (!(infile_q >> tx_q[i])) {
                std::cerr << "Ошибка при чтении данных из файла single_adalm_rx_Q_arr.txt на индексе " << i << std::endl;
                return 1;
            }
        }
        infile_i.close();
        ...
        std::cout << "Данные успешно считаны в массивы tx_i и tx_q." << std::endl;
    } else {
        std::cerr << "Не удалось открыть файлы для чтения." << std::endl;
        return 1;
    }
        ...
        */
            ...
            p_dat += p_inc / sizeof(*p_dat))
        {
            if (counter_i < 250){
                ...
                std::cout << "pd0 = " << p_dat[0] << std::endl;
                p_dat[1] = tx_q[counter_i];
                std::cout << "pd1 = " << p_dat[1] << std::endl;
            } else {
                counter_i = 0;
            }
            counter_i++;
